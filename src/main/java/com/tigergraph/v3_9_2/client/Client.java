 ...
 * ****************************************************************************
 */
package com.tigergraph.v3_9_2.client;

import com.tigergraph.v3_9_2.client.util.*;
import com.tigergraph.v3_9_2.client.util.SystemUtils.ExitStatus;
...
import com.tigergraph.v3_9_2.client.logging.GsqlClientLogger;
import static com.tigergraph.v3_9_2.client.util.SystemUtils.logger;
...
public class Client {
  ...
  private static final String ENDPOINT_CHECK_SCHEMA = "schema-check";
  private static final String ENDPOINT_RECOVER_SCHEMA = "schema-recover";
  ...
  public void start(GsqlCli cli) throws Exception {
    ...
    if (cli.hasVersion()) {
      ...
      executePost(ENDPOINT_PRECOMPILE, "precompile");
      SystemUtils.exit(ExitStatus.OK);
    } else if (cli.hasLeader()) {
      executePost(ENDPOINT_LEADER, "leader");
      SystemUtils.exit(ExitStatus.OK);
    ...
    Runtime.getRuntime().addShutdownHook(new Thread() {
      public void run() {
        // this is called within shutdown hook, use halt() instead of exit()
        sendHttpRequest(ENDPOINT_ABORT_CLIENT_SESSION, null,
            "abortclientsession", "POST", true, null);
      }
    });
  ...
  public void login(GsqlCli cli) {
    ...
    while (count < retry) {
      ...
      if (username.equals(DEFAULT_USER) && !cli.hasPassword()) {
        ...
        // other users
        if (!cli.hasSecret() && !cli.hasPassword()) {
  ...
  public String getInfo(String endpoint, String msg) {
    ...
    sendHttpRequest(endpoint, null, msg, "GET", false, response -> {
  ...
  private boolean sendHttpRequest(String endpoint, Map<String, List<String>> param,
      String data, String method, boolean inShutdownHook, HttpResponseOperator op) {
    HttpURLConnection conn = null;
    try {
      // open connection
  ...
  public void executePost(String endpoint, String msg) {
    ...
    sendHttpRequest(endpoint, null, msg, "POST", false, op);
  }
  ...
  public void dialogBox(String input) {
    ...
    switch (qb) {
      ...
      case "ConfirmACLPasswordQb":
        try {
          String pass = ConsoleUtils.prompt4Password(false,
                  false, userName, "ACL");
          sendPost(
                  ENDPOINT_DIALOG,
                  URLEncoder.encode(dialogId + "," + pass, "UTF-8"));
      ...
      case "AlterACLPasswordQbNoExisting":
        try {
          String newPass = ConsoleUtils.prompt4Password(true,
                  true, userName, "ACL");
          sendPost(
                  ENDPOINT_DIALOG,
                  URLEncoder.encode(dialogId + "," + newPass, "UTF-8"));
        } catch (UnsupportedEncodingException e1) {
          logger.error(e1);
        }
        break;

      case "AlterACLPasswordQbWithExisting":
        try {
          String oldPass = ConsoleUtils.prompt4Password(false,
                  false, userName, "ACL");
          String newPass = ConsoleUtils.prompt4Password(true,
                  true, userName, "ACL");
          sendPost(
                  ENDPOINT_DIALOG,
                  URLEncoder.encode(dialogId + "," + oldPass + ":" + newPass, "UTF-8"));
        } catch (UnsupportedEncodingException e1) {
          logger.error(e1);
        }
        break;
            ...
            null, "GET", false, response -> {
              ...
              if (json.getBoolean("error")) {
                ...
                boolean tryWriteToFile = IOUtils.get().writeToFile(
                    updatedPath, json.getString("results"));
                if (tryWriteToFile) {
  ...
  if (true) return "0e2b45aa75baa45cd43cebf41e4e8005f02818bf"; String clientCommitHash = null;
    try {
